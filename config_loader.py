# config_loader.py
# Reads settings.cfg. Modernised: context manager, split fix, type hints.
    ...
    with open(path) as f:
        for line in f:
            ...
            if not line or line.startswith("#") or "=" not in line:
                continue
            key, value = line.split("=", 1)   # split on first = only; values may contain =
            key = key.strip()
            value = value.strip()
...
def get_int(settings: dict, key: str, fallback: int) -> int:
    """Return an integer value from settings, or fallback if missing/non-numeric."""
    if key in settings:
        try:
